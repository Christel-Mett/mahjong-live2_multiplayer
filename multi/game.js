    ...
    try {
        ...
        pool = shuffleWithSeed(pool);
			...
			const finalData = [];
			    ...
			    // Versuche die Verteilung bis zu 100 Mal, falls der Algorithmus in eine Sackgasse läuft
			    while (!erfolgreichVerteilt && versuche < 100) {
			        versuche++;
			        let tempPool = [...pool];
			        // Erstelle eine tiefe Kopie der Slots und setze belegt zurück
			        ...
			        // Nur wenn alle 144 Steine (rawSlots.length) platziert wurden, ist der Versuch gültig
			...
			const loadingManager = new THREE.LoadingManager();
			    loadingManager.onLoad = () => {
			        // Sicherstellen, dass alle Steine sichtbar sind, sobald alle Texturen geladen wurden
...
socket.on('gracePeriodStarted', () => {
    if (!spielBeendet) {
        ...
        if (sidebarGraceOverlay) sidebarGraceOverlay.style.display = 'flex';
        if (graceTimer) graceTimer.style.display = 'block';
